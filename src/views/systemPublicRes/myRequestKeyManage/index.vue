<template>
  <div class="app-container calendar-list-container">
    <el-table :key='tableKey' :data="list" v-loading.body="false" border fit highlight-current-row
      ...
      <el-table-column width="300px" align="center" label="用户ID">
        <template slot-scope="scope">
          <span>{{scope.row.fromUser}}</span>
        </template>
      </el-table-column>
      <el-table-column width="300px" align="center" label="平台签名公钥" @click="">
        <template slot-scope="scope">
          <el-popover
            placement="left"
            title="平台签名公钥"
      ...
      <el-table-column width="300px" align="center" label="路径">
      <template slot-scope="scope">
...
<script>
  import {
    getAllApiJgtoPtInfo
  } from '@/api/systemPublicRes/myRequestKeyManage/index';
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        getAllApiJgtoPtInfo(this.listQuery)
          .then(response => {
